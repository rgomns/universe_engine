...
from universe_engine import run_universe, N_BITS
...
def run_single_point(args):
    ...
    if records:
        ...
        avg_strength = last.get("total_strength") / final_constraints if final_constraints > 0 else 0.0
        oldest_age = max((c["age"] for c in lineage), default=0)
    ...
    # --------------------------------------------------------
    #  DIAGNOSTICS (Option 1: computed here)
    # --------------------------------------------------------
    diagnostics = run_diagnostics(records, lineage, n_bits=N_BITS)
    summary["diagnostics"] = diagnostics

    # Save diagnostics to run folder
